...
def main():
    ...
    # dataframe
    pandas_dfs = [cgm_small, basal_small, bolus_small, activity_small, hr_small, meal_small, smbg_small]
    pandas_colnames = ['cgm', 'basal', 'bolus', 'activity', 'hr', 'meal', 'smbg']
    i = 0

    # For each data frame, coerce the values to float, remove NaNs, rename columns
    for k in pandas_dfs:
    ...
    # to averages instead
    combined_df_5_final = combined_df_5_sum[['activity', 'basal', 'bolus', 'cgm', 'hr', 'meal', 'smbg']]
    ...
    combined_df_15_final = combined_df_15_sum[['activity', 'basal', 'bolus', 'cgm', 'hr', 'meal', 'smbg']]
